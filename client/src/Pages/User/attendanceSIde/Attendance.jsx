...
const Attendance = () => {
  ...
  const isSelected = (seat) => {
    return selectedSeats.some(
      (s) => s.name === seat.name && s.serialNumber === seat.serialNumber
    );
  };
  ...
  const getallStudents = () => {
    getStudents(classnumber.class).then((data) => {
      ...
      setstudentMsg('Add Students to take attandance')
    });
  };

  const Taken = () => {
    console.log("im running");
    TakenAttandance(classnumber.class)
      .then((data) => {
        console.log(data);
  ...
  return (
    <div>
      <p className="font-bold text-4xl flex justify-center mt-2 mb-3">
        class {classnumber.class}
      </p>
      <div>
        {checkAttandance ? (
          <div className="font-bold text-3xl mt-6 flex justify-center">
            {msg}
          </div>
        ) : !Students[0] ? (
                    ...
                    seat.status?
                      <button
                        ...
                        className={classNames(
                          ...
                          {
                            "bg-green-500 text-white border-2":
                              isSelected(seat),
                            "bg-gray-200 cursor-pointer border-2  border-black":
                              !isSelected(seat),
                          }
                        )}
                        onClick={() => {
...
:<button disabled class="cursor-not-allowed inline-block  w-16 h-15 bg-gray-200  mx-2 my-2 border-2  border-black bg-orange-400">
 <p className="">{seat.serialNumber}</p>
...
<p className="text-xs ">{seat.name}</p>
      ...
      {ModalIsOpen && (
        <Confirmation
          data="your attandance has successfully submitted"
